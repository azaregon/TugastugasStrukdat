...
struct graph{
    ...
    bool dfs2(long node, long tgt, long dist, vector<bool> &visited, unordered_map<int, unordered_set<int>> &memo){
        if (dist == tgt) return true;
        if (dist > tgt) return false;
        if (memo[node].count(dist)) return false;

        // cout << node << endl;
        visited[node] = true;
        for(auto vertex:adjList[node]){
            if (!visited[vertex.first]) {
                if (dfs2(vertex.first, tgt, dist+vertex.second, visited, memo) == true) {
                    return true;
                }
            }
        }
        visited[node] = false;
        memo[node].insert(dist);
